NS_NoePaaM : NS_SynthModule {
    ...
    buildSynthModule {
        ...
        busses = (
            detune: Bus.control(nsServer.server, 1).set(0.001),
            noise:  Bus.control(nsServer.server, 1).set(0),
            rq:     Bus.control(nsServer.server, 1).set(0.5),
            gain:   Bus.control(nsServer.server, 1).set(1),
            atk:    Bus.control(nsServer.server, 1).set(0.01),
            rls:    Bus.control(nsServer.server, 1).set(0.125),
            mute:   Bus.control(nsServer.server, 1).set(1),
            amp:    Bus.control(nsServer.server, 4).setn(1!4),
            curve:  Bus.control(nsServer.server, 1).set(0), // no Control for this atm...
        );
        ...
        controlDict.addAll(
            NS_ControlFloat(\atk, ControlSpec(0.01, 0.25, \exp), 0.01)
            .addAction(\synth, { |c| busses['atk'].set(c.value) }),

            NS_ControlFloat(\rls, ControlSpec(0.1, 0.5, \exp), 0.125)
            .addAction(\synth, { |c| busses['rls'].set(c.value) }),

            NS_ControlFloat(\noise, ControlSpec(0, 1), 0)
            .addAction(\synth, { |c| busses['noise'].set(c.value) }),

            NS_ControlFloat(\rq, ControlSpec(2.sqrt.reciprocal, 0.01, \exp), 0.5)
            .addAction(\synth, { |c| busses['rq'].set(c.value) }),

            NS_ControlFloat(\detune, ControlSpec(0.001, 1, \exp), 0.001)
            .addAction(\synth, { |c| busses['detune'].set(c.value) }),

            NS_ControlFloat(\gain, ControlSpec(1, 40, \exp), 1)
            .addAction(\synth, { |c| busses['gain'].set(c.value) }),

            NS_ControlFloat(\mute, ControlSpec(0.25, 1), 1)
            .addAction(\synth, { |c| busses['mute'].set(c.value) }),

            NS_ControlInt(\harmony, 0, 4, 0)
            .addAction(\synth, { |c| 
                c.value.switch(
    ...
    nsModuleLayout {
        ^VLayout(
            ...
            NS_ControlFader(controlDict['detune'], 0.001),
            NS_ControlFader(controlDict['gain']),       
            NS_ControlFader(controlDict['mute']),       
            NS_ControlSwitch(
                controlDict['harmony'], 
                ["aFlat", "hoved", "mm13", "mm37", "mm53"], 
                5
            ), 
            NS_ControlFader(controlDict['arp_dB']),
            NS_ControlFader(controlDict['ab_dB']),
            NS_ControlButton.bypass(controlDict['playStop']),
        )
    }
