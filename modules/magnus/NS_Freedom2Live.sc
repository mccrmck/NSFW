NS_Freedom2Live : NS_SynthModule {
    ...
    buildSynthModule {
        ...
        busses = (
            dur:   Bus.control(nsServer.server, 1).set(1),
            atk:   Bus.control(nsServer.server, 1).set(0.01),
            rls:   Bus.control(nsServer.server, 1).set(1),
            curve: Bus.control(nsServer.server, 1).set(-4),
            rq:    Bus.control(nsServer.server, 1).set(0.5),
            strum: Bus.control(nsServer.server, 1).set(0),
        ...
        controlDict.addAll(
            NS_ControlFloat(\dur, ControlSpec(0.5, 2, \exp), 1)
            .addAction(\synth, { |c| busses['dur'].set(c.value) }),

            NS_ControlFloat(\atk, ControlSpec(0.01, 1, \exp), 0.01)
            .addAction(\synth, { |c| busses['atk'].set(c.value) }),

            NS_ControlFloat(\rls, ControlSpec(0.1, 2, \exp), 1)
            .addAction(\synth, { |c| busses['rls'].set(c.value) }),

            NS_ControlFloat(\crv, ControlSpec(-10, 10), -4)
            .addAction(\synth, { |c| busses['curve'].set(c.value) }),

            NS_ControlFloat(\rq, ControlSpec(0.05, 1, \exp), 0.5)
            .addAction(\synth, { |c| busses['rq'].set(c.value) }),

            NS_ControlFloat(\strum, ControlSpec(-0.5, 0.5), 0)
            .addAction(\synth, { |c| busses['strum'].set(c.value) }),

            NS_ControlFloat(\amp, \db, 0)
    ...
    nsModuleLayout {
        ^VLayout(
            NS_ControlFader(controlDict['dur']),
            NS_ControlFader(controlDict['atk']),
            NS_ControlFader(controlDict['rls']),
            NS_ControlFader(controlDict['crv']),
            NS_ControlFader(controlDict['rq']),
            NS_ControlFader(controlDict['strum']),
