    ...
    *initClass {
        StartUp.add{
            SynthDef(\ns_freezeTrig,{
                ...
                trig = trig * (1 - \trigMute.kr(0));
                trig = trig + \trig.tr(0);

                SendTrig.ar(trig,0,1);

                ReplaceOut.ar(\bus.kr * \thru.kr(1),sig)
            }).add;

            SynthDef(\ns_freeze,{
                var sig = In.ar(\bus.kr, 2).sum;
    ...
    init {
        ...
        localResponder = OSCFunc({ |msg|
            ...
            synths[0].set(\thru,1);
            synths.put(1, Synth(\ns_freeze,[\bus,bus,\bufnum, bufferArray[bufIndex], \mix,mixBus.asMap],synthGroup) );
        ...
        controls.add(
            NS_Fader("mix",ControlSpec(0,1,\amp),{ |f| mixBus.set(f.value) },initVal:1).maxWidth_(60)
        );
        ...
        controls.add(
            ...
            .action_({ |but|
                synths[0].set(\trigMute,but.value)
            })
            .valueAction_(1)
        );
